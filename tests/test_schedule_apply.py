...
def _write_candidate(vault: Path, **changes: object) -> Path:
    values: dict[str, object] = {
        ...
        "approved_at": "2026-08-15T12:00:00+00:00",
        "lifecycle": "create",
        "idempotency_key": "schedule-001",
        "area_id": "career",
        "things_tags": ["Computer", "Agenda"],
    }
...
def test_loads_only_local_schedule_apply_candidate_with_explicit_approval(tmp_path: Path) -> None:
    path = _write_candidate(tmp_path)

    candidate = _load_candidate(tmp_path, path)

    assert candidate.area_id == "career"
    assert candidate.things_tags == ("Computer", "Agenda")
    assert candidate.approved_at == datetime(2026, 8, 15, 12, 0, tzinfo=UTC)


def test_rejects_schedule_candidate_outside_manual_approval_root(tmp_path: Path) -> None:
