...
@dataclass(frozen=True, slots=True)
class EntityLandingMigrationReport:
    """Prepared link-only entity landings with separate information and history pages."""
...
def prepare_entity_landing_migration(
    vault: Path, *, migration_day: date
) -> EntityLandingMigrationReport:
    """Move entity prose below compact keyword links without dropping any content."""

    root = vault.expanduser().resolve()
    wiki_root = root / "wiki"
    if not wiki_root.is_dir():
        raise WoonError("entity landing migration requires wiki/")
    pages: dict[Path, bytes] = {}
    ...
    for path in iter_wiki_pages(wiki_root):
        ...
        if metadata.get("node_kind") != "entity" or metadata.get("entity_kind") == "book":
            continue
        entity_count += 1
        ...
        if not title or not canonical_id:
            raise WoonError(f"entity landing migration requires identity: {relative}")
        source_hashes[relative] = hashlib.sha256(path.read_bytes()).hexdigest()
        information, history = _split_entity_landing_body(body)
        landing = preserve_generated_wiki_views(text, render_markdown(metadata, f"# {title}\n"))
        pages[path] = landing.encode("utf-8")

        information_path = path.with_name(f"{path.stem}-정보.md")
        history_path = path.with_name(f"{path.stem}-히스토리.md")
        child_specs = (
            (
                information_path,
                f"{title} 정보",
        ...
        for child_path, child_title, suffix, view_mode, child_body in child_specs:
            if child_path.is_file():
                child_text = child_path.read_text(encoding="utf-8")
                child_metadata, existing_body = split_markdown(child_text)
                child_metadata["entity_section"] = suffix
                pages[child_path] = preserve_generated_wiki_views(
                    child_text, render_markdown(child_metadata, existing_body)
                ).encode("utf-8")
                continue
            ...
            child_metadata = {
                ...
                "state_reason": "entity-link-index-migration",
                "state_updated": migration_day.isoformat(),
                "summary": (
                    f"{title}의 주제별 정보다."
                    if suffix == "information"
                    else f"{title}의 날짜별 변경과 기록이다."
                ),
                "aliases": [],
                "keywords": [child_title],
                "node_kind": "detail",
                "entity_section": suffix,
                "view_mode": view_mode,
                "updated": migration_day.isoformat(),
                "parent": parent_link(relative, title),
                "people": [],
                "related_to": [parent_link(relative, title)],
                "tags": [],
            }
            pages[child_path] = render_markdown(
                child_metadata, f"# {child_title}\n\n{child_body.strip()}\n"
            ).encode("utf-8")
            created.append(child_relative)
    ...
    return EntityLandingMigrationReport(entity_count, changed, pages, manifest)


def apply_entity_landing_migration(vault: Path, report: EntityLandingMigrationReport) -> None:
    """Apply the prepared entity split atomically and validate the resulting tree."""

    root = vault.expanduser().resolve()
    receipt = root / ".local/woon-knowledge/entity-landing-migration/manifest.json"
    snapshots = {
        path: path.read_bytes() if path.is_file() else None
        for path in (*report.pages.keys(), receipt)
    }
    try:
        ...
        if tree.issues:
            raise WoonError(f"entity landing migration failed validation: {tree.issues[0]}")
        ...
        for path, previous in reversed(tuple(snapshots.items())):
            if previous is None:
                ...
                atomic_write(path, previous, mode=path.stat().st_mode & 0o777)
        raise


def _split_entity_landing_body(body: str) -> tuple[str, str]:
    """Separate explicitly chronological H2 sections from thematic information."""

    cleaned = re.sub(r"(?m)^# .+?\s*$", "", body, count=1).strip()
    if not cleaned:
        return "", ""
    matches = list(re.finditer(r"(?m)^## (?P<title>.+?)\s*$", cleaned))
    if not matches:
        return cleaned, ""
    ...
    for index, match in enumerate(matches):
        ...
        section = cleaned[match.start() : end].strip()
        heading = match.group("title").strip()
        target = history if re.search(r"(?:히스토리|이력|연혁)$", heading) else information
